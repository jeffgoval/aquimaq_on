...
/**
 * Retorna se o mês atual está dentro do período de plantio ou colheita da cultura.
 */
function isCultureInSeasonForMonth(
  row: CropCalendarRow,
  month: number
): boolean {
  const inPlant =
    month >= row.month_plant_start && month <= row.month_plant_end;
  const inHarvest =
    month >= row.month_harvest_start && month <= row.month_harvest_end;
...
export function useCropCalendar(): UseCropCalendarResult {
  ...
  return {
    cultures,
    culturesInSeasonThisMonth: Array.from(new Set(culturesInSeasonThisMonth)),
    isLoading,
    error,
  };
}
